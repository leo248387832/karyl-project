<template>
  <h4>联络方式</h4>
  <p>
    如果遇到无法解决的问题，或者您有更好的建议， 如果遇到Bug的话，可以发个邮件给我
    Email(Base64):&nbsp;aUByc3Bsd2UuY29t 或者 在b站私信我一下&nbsp;
    <a target="_blank" href="https://space.bilibili.com/44974083">(点击访问)</a
    >&nbsp;如果能再
    <del
      >顺带点个
      <b>关注</b>
    </del>
    就再好不过了(笑Cry)。
  </p>
  <h4>背景图片来源</h4>
  <p>作者: <a href="https://www.deviantart.com/arsenixc">@ArseniXC</a></p>

  <h4>Github Repo (项目地址)</h4>
  <a href="https://github.com/Rsplwe/karyl-project"
    >https://github.com/Rsplwe/karyl-project</a
  >

  <h4>引用的项目</h4>
  <ul>
    <li>
      <a href="https://github.com/yescallop/janotator">yescallop/janotator</a>
    </li>
    <li>
      <a href="https://github.com/zdhxiong/mdui">zdhxiong/mdui</a>
    </li>
    <li>
      <a href="https://github.com/beatgammit/crc32">beatgammit/crc32</a>
    </li>
    <li>
      <a href="https://github.com/fancyapps/ui">fancyapps/ui</a>
    </li>
    <li>
      <a href="https://github.com/zenorocha/clipboard.js">zenorocha/clipboard.js</a>
    </li>
    <li>
      <a href="https://github.com/lodash/lodash">lodash</a>
    </li>
    <li>
      <a href="https://github.com/MatteoGabriele/vue-gtag">MatteoGabriele/vue-gtag</a>
    </li>
    <li>
      <a href="https://github.com/vuejs/vue">vuejs/vue</a>
    </li>
    <li>
      <a href="https://github.com/axios/axios">axios</a>
    </li>
    <li>
      <a href="https://github.com/AmazingDreams/vue-matomo">AmazingDreams/vue-matomo</a>
    </li>
  </ul>

  <br />
  <p>
    Copyright &copy; 2017-2022 <a href="https://github.com/yescallop">Scallop Ye</a>,
    <a href="https://github.com/Rsplwe">Rsplwe</a>
  </p>
  <br />
  <b>願ったんなら叶えてしまえやって</b>
</template>
