<template>
  <div class="mdui-row-xs-12">
    <TextEdit
      ...
      class="mdui-col-md-3"
      label="BV号 / AV号 / 番剧ID (EP/SS)"
      placeholder="BV1XX | av12345 | ss12345 | ep12345"
    />
    <TextEdit
      v-if="state.searchMode === SearchMode.KEYWORD"
      v-model:content="keyword"
      class="mdui-col-md-9"
      label="弹幕关键 字/词"
      placeholder="如: 前方高能 (留空加载全量弹幕)"
    />
    <TextEdit
      v-else-if="state.searchMode === SearchMode.UID"
      v-model:content="uid"
      class="mdui-col-md-9"
      label="用户UID"
      type="number"
      placeholder="如: 12345 (留空加载全量弹幕)"
    />
  </div>
  <div>
    <MduiButton
    ...
    <MduiButton
      ...
      class="mdui-m-r-1 mdui-m-b-1"
      color="orange-900"
      v-on:click="changeSearchMode(SearchMode.UID)"
    >
    ...
    <MduiButton
      ...
      class="mdui-m-r-1 mdui-m-b-1"
      color="lime-900"
      v-on:click="changeSearchMode(SearchMode.KEYWORD)"
    >
  ...
  <p v-show="state.requestState">加载中，弹幕较多的情况下可能会消耗过长时间...</p>
  <br />
  <div>
    <MduiButton
      v-for="(item, i) in state.pageList"
      v-bind:key="i"
      v-on:click="loadDanmaku(item.srt)"
      class="mdui-m-r-1 mdui-m-b-1"
      color="blue-600"
      ripple
      :allow="item.srt === state.currentSelectPage"
      raised
    >
      分P{{ i + 1 }}: {{ item.name }}
    </MduiButton>
    ...
    <tbody>
      <tr v-for="(item, i) in state.danmakuList" v-bind:key="i">
        ...
        <td data-label="结果" style="color: #ff80ab">
          ...
          <div
            v-else-if="item.domState === DomState.FAILED"
            v-on:click="trackUser(item.hash, i)"
          >
            获取失败
          </div>
...
async function loadPageList() {
  ...
  const reqId = videoId.value.replace("ss", "").replace("ep", "").replace("av", "");
...
async function loadDanmaku(selectId: string) {
  ...
  try {
    ...
    state.danmakuList = _.sortBy(temp, (item) => item.playTime);
  } catch (e) {
    showDialog("Error", (e as Error).message);
  } finally {
    state.requestState = false;
